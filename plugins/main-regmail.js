...
let handler = async (m, { conn, command, text, usedPrefix }) => {
    ...
    if (id in conn.regmail) {
        conn.reply(m.chat, '*❗Ada registrasi yang belum di verifikasi!*', conn.regmail[id][0])
        throw false
    }
     if (user.registered === true) throw 'kamu sudah terdaftar jika ingin mendaftar ulang\nKetik /unreg'
  ...
  if (!isValidEmail(email)) throw `Example: ${usedPrefix + command} email`
    ...
    let res = await sendEmail(namebot, nameown, nomerown, conn.user.jid.split('@')[0], m.name, message, generateOTP, conn.user.jid.split("@")[0], avatar, email)

    let json = {
        jawaban: generateOTP,
        soal: "Masukan kode verifikasi ketik /vercode"
    }
    if (res.success == true) {
  let caption = `*${command.toUpperCase()}*
${json.soal} ${email}

Kode Expired dalam *${(timeout / 1000).toFixed(2)} detik*
    `.trim()
    let mesOtp = await conn.sendFile(m.chat, 'https://telegra.ph/file/f0e087c9a23aab897058b.jpg', 'pp.jpg', caption, m)
    
    conn.regmail[id] = [
        mesOtp,
        json, poin,
        setTimeout(() => {
            if (conn.regmail[id]) conn.reply(m.chat, `*❌ Kode verifikasi telah kedaluwarsa.*\nOTP *${json.jawaban}*`, conn.regmail[id][0])
...
handler.help = handler.command = ['regmail']
handler.tags = ['main']
...
const tqto = `- Fauzan (developed) - Zeltoria (based) -Arifzyn && Dann && yanz (scrape) -all creator Bot - Adiwajshing (bailyes)`
...
;">
      ...
      <div style="font-family:'Poppins', 'Helvetica Neue', Helvetica, Arial, sans-serif;font-size:11px;font-weight:400;line-height:24px;text-align:center;color:#445566;">${tqto}
            <br/>
            Thanks!
            <br/></div>
...
`
    try {
        return await fetch("https://send.api.mailtrap.io/api/send/", {
                method: "POST",
                headers: {
                    "Api-Token": "8377d99118b24b21e7075b830a6eeaf0",
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({
                    "from": {
                        "email": "mailtrap@apis-collaborativecrew.tech",
                        "name": "📩 Verification by " + botName
                    },
                    "to": [{
                        "email": Mail,
